Public Class Main
    ...
    Private Sub ResetForm()
        ...
        txtMembershipStatus_Write.Clear()
        txtGender_Write.Clear()
        txtMembershipType_Write.Clear()
        txtIDNumber_Write.Clear()
        txtDateIssued_Write.Clear()
        txtBranchIssued_Write.Clear()
        txtAssociateType_Write.Clear()
        txtCIF_Principal_Write.Clear()
        txtContactNos.Clear()
        txtContactName.Clear()
        txtContactContactNos.Clear()
        txtAddress.Clear()
        lblPrintTimestamp.Text = "DATE PRINTED:"
        lblMagTimestamp.Text = "DATE MAG ENCODED:"
        lblChipEncodeTimestamp.Text = "DATE CHIP ENCODED:"
    ...
    Private Sub PrintCard()
        ...
        If txtIDNumber_Write.Text.Split("_").Count <> 3 Then chipEncodeFailed = False
        If txtIDNumber_Write.Text.Contains("FAILED") Then chipEncodeFailed = False
    ...
    Private Sub btnProcessCard_Click(sender As System.Object, e As System.EventArgs) Handles btnProcessCard.Click
        ...
        If chkMagEncode.Checked Then
            ...
            If chkChipEncode.Checked Then
                Dim uid As String = WriteToCard.GetUID

                If uid = "FAILED" Then
                    MessageBox.Show("Failed to get proper card's UID. Card will be ejected.", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    EjectCard()
                    ControlDispo(True)
                    Exit Sub
                ElseIf uid = "" Then
                ...
                txtIDNumber_Write.Text = String.Format("{0}_{1}_{2}", uid.Replace(" ", ""), TerminalID, BranchCode)
                Application.DoEvents()
                System.Threading.Thread.Sleep(1000)
                ChipEncode()
            End If
    ...
    Private Sub btnReadCard_Click(sender As System.Object, e As System.EventArgs) Handles btnReadCard.Click
        ControlDispo(False)

        If Not My.Settings.PCSCReader.Contains("OMNIKEY") Then FeedCard()

        Dim ReadCard As New ReadCardData(New ListBox, TextBox1)
        If ReadCard.SuccessRead Then
            txtCIF_Write.Text = ReadCard.CIF
            txtFirst_Write.Text = ReadCard.FirstName
            txtMiddle_Write.Text = ReadCard.MiddleName
            txtLast_Write.Text = ReadCard.LastName
            txtSuffix_Write.Text = ReadCard.Suffix
            txtDOB_Write.Text = ReadCard.BirthDate
            txtMembershipDate_Write.Text = ReadCard.MembershipDate
            txtMembershipStatus_Write.Text = ReadCard.MembershipStatus
            txtGender_Write.Text = ReadCard.Gender
            txtMembershipType_Write.Text = ReadCard.MembershipType
            txtIDNumber_Write.Text = ReadCard.IDNumber
            txtDateIssued_Write.Text = ReadCard.DateIssued
            txtBranchIssued_Write.Text = ReadCard.BranchIssued
            txtAssociateType_Write.Text = ReadCard.AssociateType
            txtCIF_Principal_Write.Text = ReadCard.CIF_Principal
        Else
            SystemStatus(ReadCard.ErrorMessage, 2)
        End If
        ReadCard = Nothing
    ...
    Private Sub Button1_Click(sender As System.Object, e As System.EventArgs) Handles Button1.Click
        txtAddress.Text = GetPrinterCounter()
    End Sub
